...
When this happens, we know that the accounts B and C are more interesting than the account A with no interest rate. In addition, as there is a cost e from switching from account A to C, there will be a time where the account B is more interesting than the account C due to this transaction cost, even if the account C has a bigger return on investment (5% > 3%)
...
0.9^d[10'000(1+\frac{0.03}{365}d)] > 0.9^d[10'000(1+\frac{0.05}{365}d)] - 60  
\end{cases}
$$
...
$$
$$
60 > 0.9^d[10'000(1+\frac{0.05}{365}d)] - 0.9^d[10'000(1+\frac{0.03}{365}d)]
$$
$$
60 > 0.9^d[10'000(1+\frac{0.05}{365}d) - (10'000*(1+\frac{0.03}{365}d))]
$$
$$
60 > 0.9^d[10'000 + 10'000 * \frac{0.05}{365}d - 10'000 - 10'000*\frac{0.03}{365}d]
$$
$$
60 > 0.9^d[(10'000*\frac{0.05}{365} - 10'000*\frac{0.03}{365})*d]
$$
$$
60 > 0.9^d[\frac{200}{365}*d]
$$

She will transfer the money to the account B when there will be just 35 days remaining before her retirement. Hence, let's check what happens when there is 35 days remaining:
...
$$
$$
u(w_B) = 0.9^{35}(10'000 *(1+\frac{0.03}{365}*35)) - 0
$$
$$
u(w_B) \approx 251
$$
...
$$
$$
u(w_C) = 0.9^{35}(10'000 *(1+\frac{0.05}{365}*35)) - 60
$$
$$
u(w_C) \approx 191
$$

Hence, she will switch from the account A to the account B when there would be just 35 days remaining before she retires.
...
$$0.9^d[10'000(1+\frac{0.03}{365}d)] > 0.9^d[9940(1+\frac{0.05}{365}d)]$$
$$d < 21'900/197$$
